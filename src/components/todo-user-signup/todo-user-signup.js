...
export default function Usersignup() {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      registerDetails.password !== registerDetails.rePass
    ) {
      dispatch({
    ...
    if (selectedTab === "register") {
      ...
      if (!registerUser) {
        ...
        dispatch({
          type: "USERSDETAILS",
          usersDetails: [...registerUsersDeatils, newUser],
        });
        dispatch({
          type: "SNACKBARNOTIFICATION",
          snackBarNotification: {
        ...
        dispatch({
          ...
          snackBarNotification: {
            type: "warning",
            message: `User already register with same mailid`,
          },
        });
      }
    } else if (selectedTab === "login") {
      if (usersDetails) {
        let checkExistingUser = usersDetails.find(
          (ele) =>
            ele.mailId === e.target.email.value &&
            CryptoJS.AES.decrypt(ele.password, "clairvoyant").toString(
              CryptoJS.enc.Utf8
            ) === e.target.password.value
        );
        if (checkExistingUser) {
          dispatch({
            type: "REGISTERUSER",
            registerUser: { ...checkExistingUser, isUserLogin: true },
          });
          dispatch({
            type: "SNACKBARNOTIFICATION",
            snackBarNotification: {
        ...
        dispatch({
          type: "SNACKBARNOTIFICATION",
          snackBarNotification: { type: "error", message: `No register users` },
        });
      }
    }
  };
  ...
  return desktopView ? (
    <div className="container layout vertical">
      <div className="body layout horizontal">
        <img
          className="wall layout horizontal center-center"
          alt="todoWall"
          src={todoWall}
        ></img>
